...
def get_pos(keyPoints):
    ...
    #     # if abs(angle_left_elow) < 120 and abs(angle_right_elow) < 120:
    #     #     Str_pose = "AKIMBO"
    mouth = (np.add(keypoints[9], keypoints[10]) / 2).astype('int32')
    ...
    if keyPoints[16][1] < keyPoints[12][1] and keyPoints[15][1] < keyPoints[11][1]:
        Str_pose = "All_hands_up"
    if min(np.linalg.norm(np.subtract(mouth, keyPoints[16])),
           np.linalg.norm(np.subtract(mouth, keyPoints[15]))) < np.linalg.norm(
            np.subtract(keyPoints[11], keyPoints[12])) / 2:
        Str_pose = "Drinking"
...
def process_cap(cap, gap):
    ...
    destination[0] = (0, height / 2)
    destination[1] = (width, height / 2)
    destination[2] = (width / 2, height)
    print(destination)
    ...
    while cap.isOpened():
        ...
        if (time.time() - old_timestamp) > TIMEOUT:
            ...
            if ret:  # 判断是否读取成功
                ...
                # 绘制吃药喝水地点
                if occupied:
                    ShoulderWidth = np.linalg.norm(np.subtract(keypoints[11], keypoints[12]))
                cv.circle(frame, drinkingplace, 10, (0, 255, 255), -1)
                # 检测是否吃药喝水
                if np.linalg.norm(np.subtract(mean, drinkingplace)) < ShoulderWidth and str_pose == 'Drinking' and occupied:
                ...
                if occupied:
                    ...
                    # 判定运动方向
                    if 50 > np.linalg.norm(np.subtract(np.mean(destinationvelmean, axis=0), [0, 0, 0])) > 5:
                        cv.line(frame, destination[np.argmax(np.mean(destinationvelmean, axis=0))], tuple(mean),
                                (255, 255, 0), 3)
                else:
                    # 无人则清零
