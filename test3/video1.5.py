...
def process_frame(img):
    ...
    return img, mean, x_min, y_min, x_max, y_max, Relative_H_ofHead, aspect_ratio
        ...
        if (time.time() - old_timestamp) > TIMEOUT:
            ...
            if ret:  # 判断是否读取成功
                ...
                if bodyregcount == 0:
                    frontvel = vel
                    frontmean = mean
                    frame, mean, x_min, y_min, x_max, y_max, Relative_H_ofHead, aspect_ratio = process_frame(
                        frame)
                    SVMclf = clf.predict([[aspect_ratio, Relative_H_ofHead]])
                    if occupied == 0:
                ...
                if FallRemainedTime > 0 and int(SVMclf):
                    # 显示指示框
